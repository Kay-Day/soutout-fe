...
export const listPdf = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://103.151.238.40/api/pdf/all`);
...
export const listPdfDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://103.151.238.40/api/pdf/${id}`);
...
export const createPdfReview =
  (productId, review) => async (dispatch, getState) => {
    try {
      ...
      await axios.post(`http://103.151.238.40/api/pdf/${productId}/review`, review, config);
