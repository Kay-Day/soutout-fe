...
export const listPdf = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://soutout.online/api/pdf/all`);
...
export const listPdfDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://soutout.online/api/pdf/${id}`);
...
export const createPdfReview =
  (productId, review) => async (dispatch, getState) => {
    try {
      ...
      await axios.post(`https://soutout.online/api/pdf/${productId}/review`, review, config);
